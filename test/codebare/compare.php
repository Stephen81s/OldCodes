...
//$req = $bdd->query('SELECT * FROM prixwebmin WHERE NOT EXISTS (SELECT * FROM prixperso WHERE prixwebmin.ean=prixperso.ean);
$req = $bdd->query('SELECT ean FROM prixperso' );
$req2 = $bdd->query('SELECT ean FROM prixperso' );



      $req1 = $bdd->query('SELECT prixwebmin.ean, prixwebmin.prix as concurent, prixperso.prix as moi ,prixperso.margemin as margemin ,prixperso.prixachat as prixachat FROM prixwebmin inner join prixperso on prixwebmin.ean = prixperso.ean ' );
...
// si mon prix est superieur au meilleur prix des concurent et que celui ci est inferieur  a ma marge alors je met mon prix a ma marge
  case $prixc <= $margemin and $prix >= $prixc:
      echo ' case 1 pour le code barre '.$ean.'
            ...
            je doit donc metre le prix a '.$margemin.'<br/>';

      $req2 = $bdd->prepare('UPDATE prixperso SET prix =:margemin  WHERE ean =:ean');
...
echo 'case 4 pour le code barre '.$ean.'
      ...
      $req2 = $bdd->prepare('UPDATE prixperso SET prix =:nprix  WHERE ean =:ean');
